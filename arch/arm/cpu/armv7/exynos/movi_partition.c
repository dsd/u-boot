...
{	
	...
	raw_area_control.start_blk = 100*1024*1024/MOVI_BLKSIZE;
	...
	image[3].start_blk = image[2].start_blk + image[2].used_blk;
	...
	/* image 7 should be Recovery kernel */
	image[7].start_blk = image[6].start_blk + image[6].used_blk;
	image[7].used_blk = MOVI_RZIMAGE_BLKCNT;
	image[7].size = PART_SIZE_RKERNEL;
	image[7].attribute = 0x5;
	strcpy(image[7].description, "recoverykernel");
	dbg("rknl: %d\n", image[7].start_blk);

	/* image 8 should be Recovery */
	image[8].start_blk = image[7].start_blk + image[7].used_blk;
	image[8].used_blk = MOVI_RROOTFS_BLKCNT;
	image[8].size = PART_SIZE_RROOTFS;
	image[8].attribute = 0x6;
	strcpy(image[8].description, "recovery");
	dbg("rrfs: %d\n", image[8].start_blk);
